...
async function runTests() {
    ...
    console.log(health.success ? '✅ API is running' : '❌ API failed', health.data);
    console.log('');
    ...
    const createExpense = await apiCall('POST', '/expenses', newExpense, authToken);
    ...
    const getExpenses = await apiCall('GET', '/expenses?page=1&limit=5', null, authToken);
    ...
    if (expenseId) {
        ...
        const getExpense = await apiCall('GET', `/expenses/${expenseId}`, null, authToken);
    ...
    if (expenseId) {
        ...
        const updateExpense = await apiCall('PUT', `/expenses/${expenseId}`, {
            amount: 55.99,
    ...
    const filterExpenses = await apiCall('GET', '/expenses?category=Food&limit=5', null, authToken);
    if (filterExpenses.success) {
        console.log('✅ Filtered expenses successfully');
        console.log(`   Found ${filterExpenses.data.pagination.total} Food expenses`);
    } else {
    ...
    if (expenseId) {
        ...
        const deleteExpense = await apiCall('DELETE', `/expenses/${expenseId}`, null, authToken);
